...
export default function WalletDetails({ wallet }: { wallet: string }) {
  ...
  const [categoryAppearances, setCategoryAppearances] = useState<
    ...
    async (page: number) => {
      ...
      try {
        const response = await fetch(
          `/api/wallets/${encodeURIComponent(wallet)}?page=${page}&pageSize=${pageSize}`,
          { cache: "no-store" },
        );
        const result = await response.json();
        if (!response.ok)
          throw new Error(result.error ?? "Could not load this wallet.");
        // Page-size changes and balance saves can overlap; only the newest response
        // may update the screen, otherwise an older request can restore stale totals.
        if (request !== loadRequest.current) return;
        setData(result);
        setStartingAmounts(
          Object.fromEntries(
            result.totals.map(
              (total: { currency: string; startingAmount: number }) => [
                total.currency,
                String(total.startingAmount),
              ],
            ),
          ),
        );
  ...
  async function saveStartingAmount(currency: string) {
    ...
    try {
      const response = await fetch(
        `/api/wallets/${encodeURIComponent(wallet)}`,
        {
          method: "PUT",
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.error ?? "Could not save the starting amount.");
        ...
        {settingsOpen && (
          ...
          >
            ...
            >
              ...
              {settingsError && (
                <p className="dialog-message error">{settingsError}</p>
              )}
          ...
          <section className="ledger">
            <div className="ledger-head">
              <div>
                ...
                <p>Active transactions, newest first</p>
              </div>
            </div>
                            ...
                            <td>
                              {row.categoryName ? (
                                ...
                                >
                                  <CategoryIcon
                                    appearance={
                                      categoryAppearances[row.categoryName]
                                    }
                                  />
                                  {row.categoryName}
                                </Link>
