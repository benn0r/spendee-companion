...
type ResultRow = { key: string; date: string; description: string; detail: string; amount: number; currency: string; status: "matched" | "missing-app" | "missing-document" };
...
export default function ValidateView() {
  ...
  const load = useCallback(async () => {
    const [walletData, entries] = await Promise.all([
  ...
  const rows = useMemo<ResultRow[]>(() => {
    ...
    return [
      ...selected.diff.matching.map((item, index) => ({ key: `match-${index}`, date: item.document.date, description: item.document.description, detail: item.app.note || item.app.categoryName || "App transaction", amount: item.document.amount, currency: item.document.currency, status: "matched" as const })),
      ...selected.diff.missingInApp.map((item, index) => ({ key: `app-${index}`, date: item.date, description: item.description, detail: "Only in document", amount: item.amount, currency: item.currency, status: "missing-app" as const })),
      ...selected.diff.missingInDocument.map((item) => ({ key: `document-${item.id}`, date: item.date, description: item.note || item.categoryName || item.type, detail: "Only in app", amount: item.amount, currency: item.currency, status: "missing-document" as const })),
  ...
  return <main>
    ...
    <div className="workspace validation-page">
      <section className="page-heading"><div><p className="eyebrow">DOCUMENT RECONCILIATION</p><h1>Validate</h1><p>Compare PDF statements with the transactions saved in a wallet.</p></div><button className="page-import-button" onClick={() => setDialogOpen(true)}><span>＋</span>Upload document</button></section>
      <section className="validation-layout">
        ...
        <section className="validation-results">
          {!selected ? <div className="validation-empty"><h2>No validation selected</h2><p>Upload a PDF statement to start.</p></div> : selected.status === "processing" ? <div className="validation-empty processing"><span className="validation-spinner" /><h2>Extracting statement</h2><p>You can leave this page. The validation is running in the background.</p></div> : selected.status === "failed" ? <div className="validation-empty failed"><h2>Extraction failed</h2><p>{selected.error}</p></div> : <>
            ...
            <section className="ledger validation-ledger"><div className="ledger-head"><div><h2>Reconciliation</h2><p>All transactions in statement range</p></div><span>{rows.length} rows</span></div><div className="table-wrap"><table><thead><tr><th>Status</th><th>Description</th><th>Source</th><th className="right">Amount</th></tr></thead><tbody>{groups.map((group) => <Fragment key={group.key}><DayHeader colSpan={4} day={group.key} totals={totals[group.key] || []} />{group.rows.map((row) => <tr className={`validation-transaction ${row.status}`} key={row.key}><td><span className="validation-status-dot" /><b>{row.status === "matched" ? "Match" : row.status === "missing-app" ? "Missing in app" : "Missing in document"}</b></td><td><strong>{row.description}</strong></td><td>{row.detail}</td><td className="right"><strong>{money(row.amount, row.currency)}</strong></td></tr>)}</Fragment>)}</tbody></table></div></section>
