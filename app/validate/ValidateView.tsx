...
export default function ValidateView() {
  ...
  const filteredRows = useMemo(() => statusFilters.length === 0 ? rows : rows.filter((row) => statusFilters.includes(row.status)), [rows, statusFilters]);
  const groups = useMemo(() => groupRowsByDay(filteredRows), [filteredRows]);
  const totals = useMemo(() => calculateDayTotals(filteredRows), [filteredRows]);
