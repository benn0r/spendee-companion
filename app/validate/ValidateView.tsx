...
type ResultRow = { key: string; date: string; description: string; detail: string; amount: number; currency: string; categoryName: string | null; documentDescription: string | null; status: "matched" | "missing-app" | "missing-document" };
...
export default function ValidateView() {
  ...
  const rows = useMemo<ResultRow[]>(() => {
    ...
    return [
      ...selected.diff.matching.map((item, index) => ({ key: `match-${index}`, date: item.document.date, description: item.document.description, detail: item.app.note || item.app.categoryName || "Spendee transaction", amount: item.document.amount, currency: item.document.currency, categoryName: item.app.categoryName, documentDescription: item.document.description, status: "matched" as const })),
      ...selected.diff.missingInApp.map((item, index) => ({ key: `app-${index}`, date: item.date, description: item.description, detail: "Only in document", amount: item.amount, currency: item.currency, categoryName: null, documentDescription: item.description, status: "missing-app" as const })),
      ...selected.diff.missingInDocument.map((item) => ({ key: `document-${item.id}`, date: item.date, description: item.note || item.categoryName || item.type, detail: "Only in Spendee", amount: item.amount, currency: item.currency, categoryName: item.categoryName, documentDescription: null, status: "missing-document" as const })),
      ...
      <section className="validation-layout">
        ...
        <section className="validation-results">
          {!selected ? <div className="validation-empty"><h2>No validation selected</h2><p>Upload a PDF statement to start.</p></div> : selected.status === "processing" ? <div className="validation-empty processing"><span className="validation-spinner" /><h2>Extracting statement</h2><p>You can leave this page. The validation is running in the background.</p></div> : selected.status === "failed" ? <div className="validation-empty failed"><h2>Extraction failed</h2><p>{selected.error}</p></div> : <>
            ...
            <div className="validation-counts"><span className="matched"><b>{selected.diff.matching.length}</b> Matching</span><span className="missing-app"><b>{selected.diff.missingInApp.length}</b> Missing in Spendee</span><span className="missing-document"><b>{selected.diff.missingInDocument.length}</b> Missing in document</span></div>
            <section className="ledger validation-ledger"><div className="ledger-head"><div><h2>Reconciliation</h2><p>All transactions in statement range</p></div><span>{rows.length} rows</span></div><div className="table-wrap"><table><thead><tr><th>Status</th><th>Description</th><th>Category</th><th className="right">Amount</th><th aria-label="Actions" /></tr></thead><tbody>{groups.map((group) => <Fragment key={group.key}><DayHeader colSpan={5} day={group.key} totals={totals[group.key] || []} />{group.rows.map((row) => <tr className="validation-transaction" key={row.key}><td><span className={`validation-status-badge ${row.status}`}>{row.status === "matched" ? "Match" : row.status === "missing-app" ? "Missing in Spendee" : "Missing in document"}</span></td><td><strong>{row.description}</strong><small>{row.detail}</small></td><td>{row.categoryName ? <span className="validation-category"><CategoryIcon appearance={appearances[row.categoryName]} />{row.categoryName}</span> : "—"}</td><td className={`right validation-amount ${row.amount < 0 ? "negative" : "positive"}`}><strong>{money(row.amount, row.currency)}</strong></td><td className="validation-row-action">{row.documentDescription && <button title="Ignore this description" aria-label={`Blacklist ${row.documentDescription}`} onClick={() => void blacklistDescription(row.documentDescription!)}>⊘</button>}</td></tr>)}</Fragment>)}</tbody></table></div></section>
