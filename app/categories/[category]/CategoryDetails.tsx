...
export default function CategoryDetails({ category }: { category: string }) {
  ...
  const load = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(result.error ?? "Could not load this category.");
        setData(result);
  ...
  const chartGroups = useMemo(
    () =>
      data.chartTotals.map((total) => {
        const segments = data.segments.filter(
          (segment) => segment.currency === total.currency,
        );
        return {
          ...total,
          segments,
          magnitude: segments.reduce(
  ...
  async function saveTagSelection() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.error ?? "Could not save label selection.");
    ...
    <main>
      ...
      <div className="workspace category-page">
        <section className="category-hero">
          <div className="category-title">
            ...
            <div>
              ...
              <p>
                {data.total.toLocaleString(intlLocale)} matching{" "}
                {data.total === 1 ? "transaction" : "transactions"} across{" "}
                {data.wallets.length}{" "}
          ...
          <div className="category-summary-actions">
            <div className="category-spend">
              <small>
                Net category total ·{" "}
                {data.currentMonth
                  ? formatMonth(data.currentMonth, intlLocale)
                  : "Current month"}
              </small>
              {loading && !data.spendingTotals.length ? (
                <strong>Loading…</strong>
              ) : (
                data.spendingTotals.map((total) => (
                  <strong key={total.currency}>
                    {formatAmount(total.amount, total.currency, intlLocale)}
                  </strong>
                ))
              )}
            </div>
            ...
            >
              <div className="dialog-head">
                <div>
                  ...
                  <span>
                    Choose which insights are shown for {data.category}.
                  </span>
                </div>
                <button
              ...
              <label className="setting-toggle">
                <span>
                  <b>Spending by label</b>
                  <small>
                    Include expenses and income to show the net amount for each
                    label.
                  </small>
                </span>
                <input
                  ...
                  onChange={(event) =>
                    setSpendingByTagEnabled(event.target.checked)
                  }
                  type="checkbox"
                />
              </label>
              {spendingByTagEnabled && (
                <div className="dialog-tag-settings">
                  <div className="tag-config-head">
                    <div>
                      <b>Labels in the chart</b>
                      <span>
                        {selectedTags.length} of {data.availableTags.length}{" "}
                        selected
                      </span>
                    </div>
                    <div>
                      <button
                        type="button"
                        onClick={() => setSelectedTags(data.availableTags)}
                      >
                        Select all
                      </button>
        ...
        {error ? (
          ...
          <>
            {data.spendingByTagEnabled && (
              <section className="tag-chart-card">
                <div className="section-heading">
                  <div>
                    <h2>Spending by label</h2>
                    <p>
                      Net expenses and income; selected labels are shown
                      separately and everything else is Other
                    </p>
                  </div>
                  <label className="chart-month-select">
                    <span>Period</span>
                    <select
                      value={chartMonth || data.currentMonth}
                      onChange={(event) => setChartMonth(event.target.value)}
                    >
                      {data.availableMonths.map((month) => (
                        <option key={month} value={month}>
                          {formatMonth(month, intlLocale)}
                        </option>
                      ))}
                ...
                <p className="chart-note">
                  Income offsets expenses within each label. Pie sizes use the
                  magnitude of each resulting net amount.
                </p>
              </section>
            )}
            ...
            <section className="ledger">
              <div className="ledger-head">
                <div>
                  ...
                  <p>All matching wallets, newest first</p>
                </div>
              </div>
              <TransactionFilters
                        ...
                        <Fragment key={group.key}>
                          ...
                          {group.rows.map((row) => (
                            <tr key={row.id}>
                              ...
                              <td className="right">
                                <span
                                  className={`amount ${row.amount < 0 ? "expense" : "income"}`}
                                >
                                  {formatAmount(
                                    row.amount,
                                    row.currency,
