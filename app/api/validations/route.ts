...
export async function POST(request: Request) {
  try {
    ...
    if (!wallet)
      return NextResponse.json({ error: "Choose a wallet." }, { status: 400 });
    if (!(file instanceof File))
      return NextResponse.json(
        { error: "Choose a PDF document." },
        { status: 400 },
      );
    if (file.size > 20 * 1024 * 1024)
      return NextResponse.json(
        { error: "PDF documents must be 20 MB or smaller." },
        { status: 400 },
      );
    ...
    if (pdf.subarray(0, 5).toString() !== "%PDF-") {
      return NextResponse.json(
        { error: "The uploaded file is not a readable PDF." },
        { status: 400 },
      );
    }
    ...
    const processValidation = async () => {
      try {
        ...
        completeValidation(db, id, {
          ...
          dateTo,
          thumbnail,
          diff: compareValidationTransactions(
            filterBlacklistedTransactions(db, document.transactions),
            appTransactions,
          ),
    ...
    if (process.env.VALIDATION_BACKGROUND_IMMEDIATE === "1")
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Could not validate the document.",
      },
      { status: 400 },
    );
  }
