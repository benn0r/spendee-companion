...
export default function SplitDialog({
  ...
  const currencies = Array.from(
    new Set(transactions.map((row) => row.currency)),
  );
  ...
  async function save() {
    ...
    try {
      ...
      if (currencies.length !== 1)
        throw new Error("Selected transactions must use one currency.");
      ...
      if (
        customPositions.some(
          (position) =>
            !position.description || !Number.isFinite(position.amount),
        )
      ) {
      ...
      if (!response.ok)
        throw new Error(result.error ?? "Could not save split.");
      router.push("/splits");
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Could not save split.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <section className="custom-positions">
          <div className="split-section-head">
            <div>
              ...
              <p>Add positive or negative adjustments.</p>
            </div>
            <button
              onClick={() =>
        ...
        <div className="split-controls">
          ...
          <div className="split-summary">
            <span>
              ...
              <b>{money(total, currency, locale)}</b>
            </span>
            <span>
              <small>Final split amount</small>
              <strong>{money(finalAmount, currency, locale)}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="dialog-actions">
          <button className="cancel" disabled={saving} onClick={onClose}>
            Cancel
          </button>
          <button
            className="save"
            disabled={saving || currencies.length !== 1 || !title.trim()}
            onClick={() => void save()}
          >
