...
import { intlLocale, normalizeLocale, translate, translateUiText, type AppLocale } from "@/lib/i18n";
...
export function I18nProvider({ children, locale: requestedLocale = "en" }: { children: ReactNode; locale?: string }) {
  const [locale, setLocale] = useState<AppLocale>(() => normalizeLocale(requestedLocale));
  useEffect(() => {
    setLocale(normalizeLocale(localStorage.getItem(STORAGE_KEY) ?? requestedLocale));
  }, [requestedLocale]);
  useEffect(() => {
