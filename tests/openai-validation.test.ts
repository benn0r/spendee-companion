...
import { extractValidationDocument, requestExtraction, validationModel } from "../lib/openai-validation";
...
test("mocked extraction validates data, normalizes metadata, and preserves the raw response", async () => {
  ...
  const result = await extractValidationDocument(Buffer.from("fantasy-pdf"), "statement.pdf");

  assert.deepEqual(result.document.metadata, { period: "July 2026" });
  assert.deepEqual(result.document.transactions, extractedDocument.transactions);
  assert.deepEqual(result.rawResponse, { mocked: true, output: extractedDocument });
});

test("extraction rejects incomplete metadata and malformed transactions", async (t) => {
  const invalidDocuments = [
    { value: null, message: /invalid extraction/ },
    { value: { ...extractedDocument, title: "" }, message: /incomplete document metadata/ },
    { value: { ...extractedDocument, metadata: null }, message: /incomplete document metadata/ },
    {
      value: { ...extractedDocument, transactions: [{ ...extractedDocument.transactions[0], date: "07/01/2026" }] },
      message: /invalid transaction/,
    },
    {
      value: { ...extractedDocument, transactions: [{ ...extractedDocument.transactions[0], description: "  " }] },
      message: /invalid transaction/,
    },
    {
      value: { ...extractedDocument, transactions: [{ ...extractedDocument.transactions[0], amount: null }] },
      message: /invalid transaction/,
    },
    {
      value: { ...extractedDocument, transactions: [{ ...extractedDocument.transactions[0], currency: "chf" }] },
      message: /invalid transaction/,
    },
  ];
  ...
  for (const [index, invalid] of invalidDocuments.entries()) {
    await t.test(`invalid extraction ${index + 1}`, async () => {
      ...
      await assert.rejects(
        () => extractValidationDocument(Buffer.from("fantasy-pdf"), "statement.pdf"),
        invalid.message,
      );
    });
  }
...
test("live extraction sends the PDF schema request and accepts nested response text", async () => {
  ...
  globalThis.fetch = async (_url, init) => {
    assert.equal((init?.headers as Record<string, string>).Authorization, "Bearer fantasy-api-key");
    requestBody = JSON.parse(String(init?.body));
    return Response.json({
      output: [{ content: [{ type: "output_text", text: JSON.stringify(extractedDocument) }] }],
    });
  };

  const result = await extractValidationDocument(Buffer.from("%PDF-fantasy"), "crystal-statement.pdf");

  assert.equal(requestBody?.model, validationModel);
  assert.equal(requestBody?.input[0].content[0].filename, "crystal-statement.pdf");
...
test("OpenAI requests retry transient failures but preserve final upstream errors", async (t) => {
  await t.test("HTML gateway followed by success", async () => {
    ...
    const payload = await requestExtraction("fantasy-api-key", {}, async () => undefined) as { output_text: string };
  ...
  await t.test("retryable HTTP status", async () => {
    ...
    const payload = await requestExtraction("fantasy-api-key", {}, async (milliseconds) => {
