...
test("shared UI components render accessible fantasy-data states", () => {
  ...
  const pageSize = renderToStaticMarkup(
    React.createElement(PageSizeSelect, { value: 25, onChange() {} }),
  );
...
test("assetUrl versions public assets with the footer build id", () => {
  ...
  assert.equal(
    assetUrl("/asset.svg?size=small"),
    `/asset.svg?size=small&v=${BUILD_ID}`,
  );
});
...
test("filters and top-level client views have stable server-rendered shells", () => {
  const filters = renderToStaticMarkup(
    React.createElement(TransactionFilters, {
      options: {
        wallets: ["Moon Purse"],
        types: [],
        categories: ["Dragon Food"],
        tags: ["quest"],
        authors: [],
      },
      value: emptyFilters,
  ...
  const categoryFilters = renderToStaticMarkup(
    React.createElement(TransactionFilters, {
      options: {
        wallets: [],
        types: [],
        categories: ["Hidden Realm"],
        tags: [],
        authors: [],
      },
      value: emptyFilters,
  ...
  assert.match(
    renderToStaticMarkup(React.createElement(MonthlyReport)),
    /Monthly totals/,
  );
  assert.match(
    renderToStaticMarkup(React.createElement(SplitsView)),
    /Past splits/,
  );
  const wallet = renderToStaticMarkup(
    React.createElement(WalletDetails, { wallet: "Moon Purse" }),
  );
