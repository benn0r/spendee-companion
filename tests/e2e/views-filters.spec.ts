...
test("transaction and category filters work across wallet and category views", async ({ page }, testInfo) => {
  ...
  await filters.getByText("Wallets", { exact: true }).click();
  await filters.getByLabel(wallet, { exact: true }).check();
  await filters.getByText("Categories", { exact: true }).click();
  await filters.getByLabel("Search categories").fill(category);
  await filters.getByLabel(category, { exact: true }).check();
  await filters.getByText("Labels", { exact: true }).click();
  await filters.getByLabel("Search labels").fill("cosmic");
  await filters.getByLabel("cosmic", { exact: true }).check();
