...
test("transaction and category filters work across wallet and category views", async ({
  ...
  const { category, csv, dates, variant, wallet } = fantasyData(
    testInfo,
    "Views",
  );
  ...
  let delayedPageSizeReload = false;
  await page.route(
    `**/api/wallets/${encodeURIComponent(wallet)}?page=1&pageSize=10`,
    async (route) => {
      if (delayedPageSizeReload) return route.continue();
      delayedPageSizeReload = true;
  ...
  await expect(
    walletDialog.getByRole("button", { name: "Saved" }),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByRole("dialog", { name: "Wallet settings" })
      .getByLabel("Starting amount in CHF"),
  ).toHaveValue("250");
  ...
  await expect(
    page.getByRole("img", { name: "CHF spending pie chart" }),
  ).toBeVisible();
  const categoryFilters = page.getByRole("region", {
    name: "Transaction filters",
  });
  await expect(
    categoryFilters.getByText("Categories", { exact: true }),
  ).toHaveCount(0);
  ...
  await categoryDialog
    ...
    .uncheck();
  await categoryDialog
    .getByRole("button", { name: "Category icon 3", exact: true })
    .click();
  ...
  await expect(
    page.getByRole("heading", { name: "Spending by label" }),
  ).toHaveCount(0);
  ...
  await expect(
    persistedDialog.locator("label.setting-toggle").getByRole("checkbox"),
  ).not.toBeChecked();
  await expect(
    persistedDialog.getByRole("button", {
      name: "Category icon 3",
      exact: true,
    }),
  ).toHaveClass(/selected/);
  await expect(persistedDialog.getByLabel("Category color")).toHaveValue(
    "#7c6ee6",
  );
});
