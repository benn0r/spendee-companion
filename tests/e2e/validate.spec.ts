...
test("validates a PDF against a wallet and persists the mocked OpenAI result", async ({
  ...
  const missingSpendeeRow = page
    .locator(".validation-status-badge.missing-app")
    .locator("xpath=ancestor::tr");
  ...
  await expect(matchedRow.locator(".validation-amount")).toHaveClass(
    /negative/,
  );
  ...
  await expect(page.locator(".validation-transaction")).toContainText(
    "Nebula lunch",
  );
...
test("links matched transactions to their exact validation and document description", async ({
  ...
  const { csv, wallet, variant } = fantasyData(
    testInfo,
  ...
  const matchedValidationId = await uploadMockStatement(
    page,
    wallet,
    "matched-statement.pdf",
  );
  const newerValidationId = await uploadMockStatement(
    page,
    `Cloud Vault ${variant}`,
    "newer-unmatched-statement.pdf",
  );
  ...
  const validationLink = transactionRow.getByRole("link", {
    name: "Open validation Moon Guild Card Statement",
  });
  const descriptionLine = transactionRow.locator(
  ...
  const validationDescription = transactionRow.locator(
    ".transaction-validation-description",
  );
  await expect(validationDescription).toHaveText("Nebula lunch");
  await expect(validationDescription).toHaveCSS(
    "background-color",
    "rgba(0, 0, 0, 0)",
  );
  ...
  await expect(validationLink).toHaveAttribute(
    "href",
    `/validate?validation=${matchedValidationId}`,
  );
  await expect(validationLink).toHaveText("↗");
  await expect(validationLink).toHaveCSS(
    "background-color",
    "rgba(0, 0, 0, 0)",
  );
  ...
  await expect(
    transactionRow.getByText("Statement:", { exact: true }),
  ).toHaveCount(0);
  await expect(
    transactionRow.getByText("View validation", { exact: true }),
  ).toHaveCount(0);
  ...
  await expect(page).toHaveURL(
    new RegExp(`/validate\\?validation=${matchedValidationId}$`),
  );
  await expect(
    page.locator('.validation-history button[aria-pressed="true"]'),
  ).toHaveAttribute("data-validation-id", String(matchedValidationId));
  await expect(
    page.getByRole("heading", { name: "Moon Guild Card Statement" }),
  ).toBeVisible();
