...
test("public assets use the same version as the footer build", async ({ page }) => {
  ...
  const iconUrls = await assetLinks.evaluateAll((links) => links.map((link) => (link as HTMLLinkElement).href));
  const cornerAlphas = await page.evaluate(async (urls) => Promise.all(urls.map(async (url) => {
