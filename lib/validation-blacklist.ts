...
export function listValidationBlacklist(db: Db) {
  return db
    ...
    FROM validation_description_blacklist ORDER BY description COLLATE NOCASE`,
    )
    .all() as Array<{
    id: number;
    description: string;
...
export function addValidationBlacklist(db: Db, description: string) {
  ...
  db.prepare(
    `INSERT INTO validation_description_blacklist (description, normalized_description)
...
export function deleteValidationBlacklist(db: Db, id: number) {
  return (
    db
      .prepare("DELETE FROM validation_description_blacklist WHERE id = ?")
      .run(id).changes > 0
  );
}

export function filterBlacklistedTransactions(
  db: Db,
  ...
  const blacklist = new Set(
    (
      db
        .prepare(
          "SELECT normalized_description FROM validation_description_blacklist",
        )
  ...
  return transactions.filter(
    (transaction) =>
      !blacklist.has(normalizeBlacklistedDescription(transaction.description)),
  );
}
