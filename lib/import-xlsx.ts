...
function asDate(value: unknown): string {
  ...
  if (!text || Number.isNaN(date.valueOf()))
...
function parseCells(cells: unknown[][]) {
  if (!cells.length)
    throw new Error("The file does not contain a readable transaction table.");
  ...
  const indexes = Object.fromEntries(
    cells[0].map((value, index) => [String(value ?? "").trim(), index]),
  );
  return cells
    .slice(1)
    .filter((row) => row.some((value) => value != null))
    .map((row, index) => {
      const get = (column: (typeof transactionColumns)[number]) =>
        row[indexes[column]];
      ...
      const currency = String(get("Currency") ?? "")
        .trim()
        .toUpperCase();
      if (!wallet || !type || !currency)
        throw new Error(
          `Row ${index + 2}: wallet, type and currency are required`,
        );
      if (!/^[A-Z]{3}$/.test(currency))
        throw new Error(
          `Row ${index + 2}: currency must be a three-letter code`,
        );
      ...
      const raw = Object.fromEntries(
        transactionColumns.map((column) => [column, get(column)]),
      );
