...
function translateEnglishDate(locale: Exclude<AppLocale, "en">, text: string) {
  ...
  if (match) {
    ...
    if (locale === "pt-BR")
      return `${day ? `${day} de ` : ""}${months[match[2]]}${year ? ` de ${year}` : ""}`;
    return `${day ? `${day} ` : ""}${months[match[2]]}${year ? ` ${year}` : ""}`;
  }
  ...
  if (locale === "pt-BR")
    return `${match[1]} de ${month} de ${match[3]}${match[4]}`;
...
export function intlLocale(locale: AppLocale) {
  return (
    supportedLocales.find((item) => item.code === locale)?.intlLocale ?? "en-CH"
  );
}
