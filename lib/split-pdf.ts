...
export function createSplitPdf(split: SplitPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const summaries = [
      ...
      [
        t("split.pdf.finalAmount"),
        money(split.splitAmount, split.currency, locale),
      ],
    ];
    ...
    document
      .fillColor(muted)
      .fontSize(7)
      .text(t("split.pdf.date"), 48, tableHeaderY, {
        width: 70,
        lineBreak: false,
      })
      .text(t("split.pdf.description"), 126, tableHeaderY, {
        width: 260,
        lineBreak: false,
      })
      .text(t("split.pdf.amount"), 400, tableHeaderY, {
        width: 147,
        align: "right",
        lineBreak: false,
      });
    ...
    for (const entry of split.entries) {
      ...
      const details =
        entry.kind === "custom"
          ...
          : [entry.wallet, entry.categoryName].filter(Boolean).join("  |  ");
      document
        .fillColor(muted)
        .font("Helvetica")
        .fontSize(7)
    ...
    for (
      ...
      document
        ...
        .fontSize(7)
        .text(
          `${t("app.name")}  |  ${split.title}  |  ${t("split.pdf.page", { page: index + 1 })}`,
          48,
          785,
          {
            width: 499,
