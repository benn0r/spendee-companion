...
const extractionSchema = {
  ...
  required: ["title", "printDate", "issuer", "accountReference", "documentCurrency", "metadata", "transactions"],
  properties: {
    title: { type: "string" },
    printDate: { type: ["string", "null"], description: "ISO date YYYY-MM-DD when present" },
    ...
    transactions: {
      ...
      items: {
        ...
        properties: {
          ...
          amount: { type: "number", description: "Signed amount: expenses negative, credits/payments positive" },
...
function validateExtraction(value: unknown): ExtractedDocument {
  ...
  if (!candidate.title?.trim() || !Array.isArray(candidate.transactions) || !candidate.metadata || typeof candidate.metadata !== "object") {
    throw new Error("OpenAI returned incomplete document metadata.");
  }
  const document: ExtractedDocument = {
    ...candidate,
    metadata: Array.isArray(candidate.metadata)
      ? Object.fromEntries(candidate.metadata.map((item) => [item.key, item.value]))
      : candidate.metadata,
  };
...
export async function extractValidationDocument(pdf: Buffer, filename: string): Promise<{
  document: ExtractedDocument;
  rawResponse: unknown;
}> {
  if (process.env.OPENAI_VALIDATION_MOCK) {
    const mock = JSON.parse(process.env.OPENAI_VALIDATION_MOCK);
    return { document: validateExtraction(mock), rawResponse: { mocked: true, output: mock } };
  }
